...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("TemerCoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #TemerCoinTEST2\r");
            Send(hSocket, "WHO #TemerCoinTEST2\r");
        } else {
            // randomly join #TemerCoin00-#TemerCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #TemerCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #TemerCoin%02d\r", channel_number).c_str());
        }

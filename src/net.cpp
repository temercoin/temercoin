...
static const char *strDNSSeed[][2] = {
    {"seed.fenixhash.com", "seed.fenixhash.com"}
};

void ThreadDNSAddressSeed(void* parg)
{
    // Make this thread recognisable as the DNS seeding thread
    RenameThread("TemerCoin-dnsseed");
...
void ThreadDumpAddress(void* parg)
{
    ...
    RenameThread("TemerCoin-adrdump");
...
void ThreadOpenConnections(void* parg)
{
    ...
    RenameThread("TemerCoin-opencon");
...
void ThreadMessageHandler(void* parg)
{
    ...
    RenameThread("TemerCoin-msghand");
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. TemerCoin is probably already running."), addrBind.ToString().c_str());
        else

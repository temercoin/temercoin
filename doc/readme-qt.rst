TemerCoin-qt: Qt4 GUI for TemerCoin
===============================
...
Alternatively, install Qt Creator and open the `TemerCoin-qt.pro` file.

An executable named `TemerCoin-qt` will be built.
...
A warning for people using the *static binary* version of TemerCoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of TemerCoin is linked against libdb5.3.
...
installed causes TemerCoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
